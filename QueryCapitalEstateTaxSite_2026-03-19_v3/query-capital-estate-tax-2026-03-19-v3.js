...
async function initialize() {
  const response = await fetch("./data/query-capital-estate-tax-2026-03-19-v3.json");
...
function buildGovernmentSourceHtml(stateName, stateResult) {
  ...
  return `${prefix}<a class="source-link" href="${source.url}" target="_blank" rel="noreferrer">${escapeHtml(label)}</a>`;
}
