...
const IRS_STATE_DIRECTORY_BASE_URL =
...
async function initialize() {
  const response = await fetch("./data/query-capital-estate-tax-2026-03-19-v7.json");
  ...
  stateSelect.addEventListener("input", render);
...
function render() {
  ...
  setResults({
    ...
    overviewHtml: buildOverviewHtml(stateEntry, stateName),
    notesHtml: buildNotesHtml(stateEntry, stateName, stateResult),
  });
}
...
function setResults({
  ...
  stateOverview.innerHTML = overviewHtml;
  stateNotes.innerHTML = notesHtml;
}
...
function buildOverviewHtml(stateEntry, stateName) {
  if (!stateEntry || !stateName) {
    return "Select a state to see its overview values.";
  }

  return [
    `<span>Estate tax exemption: ${escapeHtml(formatRawValue(stateEntry.estateTaxExemption))}.</span>`,
    `<span>Estate tax rate: ${escapeHtml(formatOverviewRate(stateEntry.estateTaxRate))}.</span>`,
    `<span>Inheritance tax exemption: ${escapeHtml(formatRawValue(stateEntry.inheritanceTaxExemption))}.</span>`,
    `<span>Inheritance tax rate: ${escapeHtml(formatOverviewRate(stateEntry.inheritanceTaxRate))}.</span>`,
    `<span class="overview-method">${escapeHtml(buildStateMethodText(stateEntry, stateName))}</span>`,
  ].join("");
}

function buildNotesHtml(stateEntry, stateName, stateResult) {
  if (!stateName) {
    return "Select a state to see workbook notes and official source.";
  }

  const noteText = stateEntry?.notes || "No additional notes available for this state.";
  const source = GOVERNMENT_SOURCES[stateName] || buildDefaultStateSource(stateName);
  const sourcePrefix =
    stateResult.reason === "missing-sheet"
      ? "No state estate tax table is used for this state. "
      : "";

  return [
    `<span class="notes-method">${escapeHtml(noteText)}</span>`,
    `<span class="notes-source">${escapeHtml(sourcePrefix)}Official source: <a class="source-link" href="${source.url}" target="_blank" rel="noreferrer">${escapeHtml(source.label)}</a></span>`,
  ].join("");
}

function buildStateMethodText(stateEntry, stateName) {
  const table = taxData.tables[stateName];
  const exclusion = findExclusionThreshold(table);
  const exclusionText = Number.isFinite(exclusion) ? formatCurrency(exclusion) : "the exclusion";

  if (Number(parseNumericRate(stateEntry?.estateTaxRate)) <= 0) {
    return `No estate tax in ${stateName}.`;
  }

  if (stateName === "Illinois" || stateName === "New York") {
    return "Cliff method: over the exclusion, the entire estate is taxable.";
  }

  if (stateName === "Vermont") {
    const rate = findTopRate(table);
    return `Flat tax: The amount over ${exclusionText} is taxed at a flat ${formatPercent(rate)}.`;
  }

  if (stateName === "Connecticut") {
    const rate = findTopRate(table);
    return `Flat tax: The amount over ${exclusionText} is taxed at a flat ${formatPercent(rate)}. Exclusion syncs with federal law.`;
  }

  if (Number(parseNumericRate(stateEntry?.estateTaxRate)) > 0) {
    return "Marginal tax brackets: The estate value over the exclusion is taxed.";
  }

  return `No estate tax in ${stateName}.`;
}

function findExclusionThreshold(table) {
  if (!table?.brackets?.length) {
    return NaN;
  }

  const thresholdBracket = table.brackets.find(
    (bracket) => Number.isFinite(bracket.threshold) && bracket.threshold > 0 && Number(bracket.rate) > 0
  );

  return thresholdBracket ? thresholdBracket.threshold : NaN;
}

function findTopRate(table) {
  if (!table?.brackets?.length) {
    return 0;
  }

  return table.brackets.reduce((highest, bracket) => {
    const rate = Number(bracket.rate);
    return Number.isFinite(rate) && rate > highest ? rate : highest;
  }, 0);
}
...
function parseNumericRate(value) {
  ...
  const firstPart = text.split("–")[0].split("-")[0].replace("%", "").trim();
...
function buildDefaultStateSource(stateName) {
  ...
  return {
    label: `${stateName} government tax links via IRS`,
    url: `${IRS_STATE_DIRECTORY_BASE_URL}/${slug}`,
  };
}
...
function formatOverviewRate(value) {
  return formatPercent(parseNumericRate(value));
}
