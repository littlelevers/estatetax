...
async function initialize() {
  const response = await fetch("./data/query-capital-estate-tax-2026-03-19-v6.json");
...
function buildOverviewText(stateEntry) {
  ...
  return [
    ...
    `Estate tax rate: ${formatOverviewRate(stateEntry.estateTaxRate)}.`,
    `Inheritance tax exemption: ${formatRawValue(stateEntry.inheritanceTaxExemption)}.`,
    `Inheritance tax rate: ${formatOverviewRate(stateEntry.inheritanceTaxRate)}.`,
  ].join(" ");
}
...
function buildNotesHtml(stateEntry, stateName, stateResult) {
  ...
  const noteText = buildStateMethodText(stateEntry, stateName, stateResult);
